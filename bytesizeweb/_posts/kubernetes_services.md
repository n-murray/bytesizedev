---
title: "A Beginners Guide to Understanding Kubernetes: Services"
excerpt: "A beginner-friendly overview of services in Kubernetes and the different types you may encounter."
coverImage: "https://miro.medium.com/v2/format:webp/1*nOwzQBOPoGEds5aU-NiErw.png"
date: "2025-01-07"
author:
  name: Nicholas Murray
  picture: "/assets/profile.jpeg"
ogImage:
  url: "https://miro.medium.com/v2/format:webp/1*nOwzQBOPoGEds5aU-NiErw.png"
---
...
spec:
  selector:
    app: backend # Telling the selector which label to look for
  ports:
    - name: http-service-port
      protocol: TCP
      port: 80 # Traffic will be received on this port
      targetPort: api # The port to forward traffic to, this can be a name or number
---
apiVersion: v1
kind: Pod
metadata:
  name: backend
  labels:
    app: backend # Assigning the label to our Pod
spec:
  containers:
  - name: backend
    image: nginx:stable
...
spec:
  ...
  ports:
    - name: http-service-port
      ...
      port: 80
      targetPort: api
      nodePort: 32500 # Only required when manually assigning a port.
```
...
metadata:
  name: backend-service
  annotations:
    metallb.universe.tf/loadBalancerIPs: 192.168.68.1  # External loadbalancer specific annotation
spec:
  selector:
    app: backend
  type: LoadBalancer
...
kind: Service
metadata:
  name: external-applcation
spec:
  type: ExternalName
  externalName: legacy.application.com
```
